...
def train(model, salary_json, seq_length=4, batch_size=20, epochs=200, lr=1e-3, save:bool=True):
    model.train()
    salaries = []

    for dictionary in salary_json.get("salaries"):
        for month, salary in dictionary.items():
            salaries.append(salary)

    train_tensor = torch.FloatTensor(salaries).view(-1, 1)
    ...
    if save:
        print("TTTTTT")
        # path = os.path.join(settings.BASE_DIR, 'main/ml_model', "/trained_model.pth")

        model.save(str(settings.BASE_DIR )+ "/main/ml_model/" + "trained_model.pth")
    return model

def savedModelForecast(model, salary_json, dataframe_path = "./dataset.csv", path="trained_model.pth", column="salary", future=100, seq_length=300):
    # dataframe_path = os.path.join(settings.BASE_DIR, 'main', dataframe_path)
    path = os.path.join(settings.BASE_DIR, 'main/ml_model', path)
    ...
    data_tensor = torch.FloatTensor(salaries).view(-1, 1)
